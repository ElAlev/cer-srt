...
def create_wayeb_query():
    ...
    for i in QUERIES:
        with open(f'{WORKING_FOLDER}/taxiqueries/WAYEB/kn{i}.sre') as tf:
            original = tf.read()
        for win_length in WIN_LENGTH:
            with open(f'{WORKING_FOLDER}/results/{TEST_NAME}/wayeb/wayeb_taxi_kn{i}_{win_length}.sre', 'w') as tf:
...
def create_esper_query():
    ...
    for i in QUERIES:
        with open(f'{WORKING_FOLDER}/taxiqueries/ESPER/kn{i}.query') as tf:
            original = tf.read()
        for win_length in WIN_LENGTH:
            with open(f'{WORKING_FOLDER}/results/{TEST_NAME}/esper/esper_taxi_kn{i}_{win_length}.query', 'w') as tf:
                tf.write(original.replace('TIMESTAMP', f'{win_length}'))
                    ...
                    for j in range(len(TIMEOUTS)):
                        ...
                        for i in range(ITERATIONS):
                            try:
                                if system == 'wayeb':
                                    print(
                                        f'Running wayeb with query wayeb_taxi_kn{query}_{win_length}.sre, stream taxi.stream. Memorytest: {memorytest}')
                                    t0 = time.time_ns()
                                    res = run_wayeb(win_length, query, memorytest, timeout)
                                    total_time = time.time_ns() - t0
                                elif system == 'esper8':
                                    print(
                                        f'Running esper8 with query esper_taxi_kn{query}_{win_length}.query, stream taxi.stream. Memorytest: {memorytest}')
                                    t0 = time.time_ns()
                                    res = run_esper8(win_length, query, memorytest, timeout, max_events)
                                    total_time = time.time_ns() - t0
                                else:
                                    continue
                                with open(f'{WORKING_FOLDER}/results/{TEST_NAME}/results/{system}_taxi_kn{query}_{win_length}_{test}.query' + '_out.txt', 'ab') as tf:
                                    if not j and not i:
                                        if memorytest:
                                            tf.write(
                                    ...
                                    tf.write(res.stdout)
                                with open(f'{WORKING_FOLDER}/results/{TEST_NAME}/results/{system}_taxi_kn{query}_{win_length}_{test}.query' + '_err.txt', 'ab') as tf:
                                    tf.write(res.stderr)
                                print(
                                    f'successfully ran {system} query {system}_taxi_kn{query}_{win_length}.query with stream taxi.stream.')
                            except subprocess.TimeoutExpired as err:
                                with open(f'{WORKING_FOLDER}/results/{TEST_NAME}/results/{system}_taxi_kn{query}_{win_length}_{test}.query' + '_except.txt', 'a') as tf:
...
def run_wayeb(win_length, query, memorytest, timeout):
    subprocess.run(['java', '-Xmx50G',
                    ...
                    'compile',
                    f'--patterns:{WORKING_FOLDER}/results/{TEST_NAME}/wayeb/wayeb_taxi_kn{query}_{win_length}.sre',
                    f'--fsmModel:{FSM_MODEL}',
                    f'--outputFsm:{WORKING_FOLDER}/results/{TEST_NAME}/wayeb/wayeb_taxi_kn{query}_{win_length}.fsm'],
                    timeout=timeout * 10, capture_output=False, check=True)
    return subprocess.run(['java', '-Xmx50G',
                           '-jar', f'{WORKING_FOLDER}/jars/wayeb.jar',
                           'recognition',
                           f'--fsm:{WORKING_FOLDER}/results/{TEST_NAME}/wayeb/wayeb_taxi_kn{query}_{win_length}.fsm',
                           f'--fsmModel:{FSM_MODEL}',
...
def run_esper8(win_length, query, memorytest, timeout, max_events):
    return subprocess.run(['java', '-Xmx50G',
                           ...
                           'taxi',
                           f'{WORKING_FOLDER}/results/{TEST_NAME}/esper/esper_taxi_kn{query}_{win_length}.query',
