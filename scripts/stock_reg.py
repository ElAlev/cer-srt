...
def create_sase_query():
    ...
    for i in QUERIES:
        with open(f'{WORKING_FOLDER}/stockqueries/SASE/reg{i}.query') as tf:
            original = tf.read()
...
def create_wayeb_query():
    ...
    for i in QUERIES:
        with open(f'{WORKING_FOLDER}/stockqueries/WAYEB/reg{i}.sre') as tf:
            original = tf.read()
        for win_length in WIN_LENGTH:
...
def create_esper_query():
    ...
    for i in QUERIES:
        with open(f'{WORKING_FOLDER}/stockqueries/ESPER/reg{i}.query') as tf:
            original = tf.read()
        for win_length in WIN_LENGTH:
